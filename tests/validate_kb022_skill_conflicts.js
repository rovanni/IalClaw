...
const CognitiveOrchestrator_1 = require("../src/core/orchestrator/CognitiveOrchestrator");
const AgentLoop_1 = require("../src/engine/AgentLoop");
async function validateConflicts() {
    console.log('🧪 Iniciando Validação Dirigida - KB-022 (Skill Flow Conflicts)\n');
    // Mocks mínimos
    const mockMemory = {};
    const mockFlowManager = { isInFlow: () => false };
    const orchestrator = new CognitiveOrchestrator_1.CognitiveOrchestrator(mockMemory, mockFlowManager);
    ...
    orchestrator.ingestSignalsFromLoop({
        route: {
            ...
            route: AgentLoop_1.ExecutionRoute.TOOL_LOOP,
    ...
    orchestrator.ingestSignalsFromLoop({
        ...
        route: {
            recommendedStrategy: 'EXECUTE',
            route: AgentLoop_1.ExecutionRoute.DIRECT_LLM,
            reason: 'Resposta direta'
        }
    }, sessionId);
    decisions = orchestrator.applyActiveDecisions(sessionId);
    if (decisions.applied.fallback?.fallbackRecommended && decisions.applied.route?.route === AgentLoop_1.ExecutionRoute.DIRECT_LLM) {
